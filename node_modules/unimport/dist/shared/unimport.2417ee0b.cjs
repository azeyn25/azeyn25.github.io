...
const vueTemplate = require('./unimport.b654920c.cjs');
const fs = require('fs');
...
function createUnimport(opts) {
  ...
  function updateImports() {
    if (!_combinedImports) {
      let imports = vueTemplate.normalizeImports(vueTemplate.dedupeImports([...ctx.staticImports, ...ctx.dynamicImports], opts.warn || console.warn));
...
async function injectImports(code, id, ctx, options) {
  ...
  return {
    ...vueTemplate.addImportToCode(
      ...
      (imports2) => {
        for (const addon of ctx.addons) {
          imports2 = addon.injectImportsResolved?.call(ctx, imports2, s) ?? imports2;
        }
        return imports2;
      },
      (str, imports2) => {
        for (const addon of ctx.addons) {
          str = addon.injectImportsStringified?.call(ctx, str, imports2) ?? str;
        }
        return str;
      }
    ),
